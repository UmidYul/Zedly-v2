...
> Статус: **Resolved in docs v1.1**. Ниже сохранён архив первичных расхождений, которые были выявлены до исправлений.
...
## Архив: первично найденные расхождения (до v1.1)
...
## Дополнительная проверка качества формулировок (prompt quality)

### Явные проблемы

1. **Риск ложного чтения статуса документа.**
   Вверху указан статус `Resolved`, но ниже идёт большой блок в настоящем времени «Найденные несоответствия», что можно ошибочно трактовать как актуальные проблемы.

2. **Смешение типов контента в одном документе.**
   В одном файле объединены:
   - отчёт об уже внедрённых исправлениях;
   - исходный список найденных дефектов;
   - рекомендации на будущее.
   Из-за этого документ сложно использовать как источник «текущего состояния».

3. **Нет дат/версий у отдельных блоков.**
   Для аудита и follow-up полезно явно помечать, к какой версии/дате относится каждый раздел.

### Предложенные улучшения

1. **Разделить отчёт на две сущности:**
   - `consistency_audit_2026-02-27.md` — только итоговый статус после фиксов;
   - `consistency_audit_2026-02-27_findings.md` — архив первичных findings.

2. **Ввести обязательный шаблон для подобных документов:**
   - `Scope`;
   - `Version checked`;
   - `Current status (Open/Resolved/Partially resolved)`;
   - `Evidence (file links)`;
   - `Decision / Follow-up owner / Due date`.

3. **Нормализовать формулировки статусов:**
   использовать единые маркеры (`OPEN`, `RESOLVED`, `DEFERRED`) вместо свободного текста.
